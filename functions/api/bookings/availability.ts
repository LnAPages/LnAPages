...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const { results } = await env.FNLSTG_DB.prepare('SELECT start_time FROM bookings WHERE start_time BETWEEN ? AND ? AND status != ?').bind(dayStart, dayEnd, 'cancelled').all<{ start_time: string }>();
  const used = new Set((results ?? []).map((row) => new Date(row.start_time).toISOString().slice(11, 16)));
