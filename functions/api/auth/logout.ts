...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
    ...
    if (rawToken) {
      ...
      if (row) {
        await env.LNAPAGES_DB
          ...
          .bind(row.id)
          .run();
        await writeAuditLog(env.LNAPAGES_DB, 'auth.logout', { userId: row.user_id, ip, ua });
      }
    }
  }
  ...
  response.headers.append('Set-Cookie', 'LNAPAGES_session=; Path=/; Max-Age=0; HttpOnly; Secure; SameSite=Lax');
  response.headers.append('Set-Cookie', 'LNAPAGES_csrf=; Path=/; Max-Age=0; Secure; SameSite=Lax');
