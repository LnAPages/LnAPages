...
async function ensureProductExists(env: Env, id: number): Promise<void> {
  const product = await env.LNAPAGES_DB.prepare('SELECT id FROM products WHERE id = ?').bind(id).first<{ id: number }>();
  if (!product) {
...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  const statements = [
    context.env.LNAPAGES_DB.prepare('DELETE FROM product_tags WHERE product_id = ?').bind(id),
    ...tags.map((tag) =>
      context.env.LNAPAGES_DB.prepare('INSERT OR IGNORE INTO product_tags (product_id, tag) VALUES (?, ?)').bind(id, tag),
    ),
  ];
