# FNL STAGE booking + admin scaffold

Production-ready scaffold for **fnlstage.com** built with Vite + React + Cloudflare Pages Functions + D1 + R2 + KV.
...
```bash
wrangler d1 create fnlstg
wrangler r2 bucket create fnlstg-gallery
wrangler kv namespace create fnlstg-config
```
...
For Cloudflare Pages runtime, set both `STRIPE_SECRET_KEY` and `STRIPE_PUBLISHABLE_KEY` as secret bindings. A build-time `VITE_STRIPE_PUBLISHABLE_KEY` is no longer required.

## Database migrations
Local:
```bash
wrangler d1 migrations apply fnlstg --local
```
Remote:
```bash
wrangler d1 migrations apply fnlstg --remote
```
CI/deploy step (generic):
```bash
...
- [ ] D1 binding: `FNLSTG_DB`
- [ ] R2 binding: `FNLSTG_GALLERY`
- [ ] KV binding: `FNLSTG_CONFIG`
- [ ] Env vars from `.env.example`
- [ ] `nodejs_compat` compatibility flag

## R2 CORS setup
Configure your R2 bucket CORS to allow `PUT` and `GET` from your app domain and localhost during development.

<!-- trigger redeploy -->

<!-- redeploy 1777179589744 -->
